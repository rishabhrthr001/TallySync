...
async function retry() {
    try {
        ...
        const target = entriesRes.data.find(e => e.invoiceNumber === 'INV-9612' && e.companyName === 'Nikhil');
        if (target) {
            console.log(`Retrying entry ${target._id} (INV-9612)...`);
            await axios.patch(`${CONFIG.BACKEND_URL}/api/entries/${target._id}/sync-status`, {
                status: 'pending'
            }, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            ...
            console.log('Entry INV-9612 for Nikhil not found.');
        }
    } catch (err) {
