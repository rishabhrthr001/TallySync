 ...
 */
const CONFIG = {
    BACKEND_URL: process.env.BACKEND_URL || 'https://tallysync-backend-1020363630918.asia-south1.run.app',
    TALLY_URL: process.env.TALLY_URL || 'http://127.0.0.1:9000',
    EMAIL: process.env.AGENT_EMAIL || 'pankaj@tallySync.com',
...
<TDL><TDLMESSAGE><COLLECTION NAME="TallySyncLedgers"><TYPE>Ledger</TYPE><FETCH>NAME</FETCH></COLLECTION></TDLMESSAGE></TDL>
</DESC></BODY></ENVELOPE>`;
    try {
        const data = await tallyRequest(xml);
...
<TDL><TDLMESSAGE><COLLECTION NAME="TallySyncStockItems"><TYPE>Stock Item</TYPE><FETCH>NAME, BASEUNITS</FETCH></COLLECTION></TDLMESSAGE></TDL>
</DESC></BODY></ENVELOPE>`;
    try {
        const data = await tallyRequest(xml);
...
function generateInventoryVoucherXML(entry, partyName, incomeLedger, taxLedgers) {
    ...
    (entry.items || []).forEach(item => {
        ...
        const qtyStr     = ` ${itemQty} ${escapeXML(item.uom || 'Nos')}`;
