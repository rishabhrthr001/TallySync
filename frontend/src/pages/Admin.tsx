...
export default function Admin() {
  ...
  const fetchData = async () => {
    try {
      ...
      let usersData = [];
      try {
        const subRes = await axios.get('/api/subscription/admin/list', { headers });
        if (subRes.data.success) {
          usersData = subRes.data.users;
        } else {
      ...
      const entriesRes = await axios.get('/api/entries', { headers });
      setUsers(usersData);
  ...
  const handleGrantTrial = async (userId: string) => {
    try {
      const headers = { Authorization: `Bearer ${localStorage.getItem('token')}` };
      const res = await axios.post(`/api/subscription/admin/${userId}/grant-trial`, { days: 30 }, { headers });
      showToast(res.data.message || 'Granted 30-Day Free Trial');
      fetchData();
    } catch (err: any) {
      showToast(err.response?.data?.error || 'Failed to grant trial', 'error');
    }
  };
  ...
  const handleUpgradePro = async (userId: string) => {
    try {
      const headers = { Authorization: `Bearer ${localStorage.getItem('token')}` };
      const res = await axios.post(`/api/subscription/admin/${userId}/upgrade-pro`, { days: 30 }, { headers });
      showToast(res.data.message || 'Upgraded to Pro Package (₹299/mo)');
      fetchData();
    } catch (err: any) {
      showToast(err.response?.data?.error || 'Failed to upgrade to Pro', 'error');
    }
  };

  const handleResetFree = async (userId: string) => {
    try {
      const headers = { Authorization: `Bearer ${localStorage.getItem('token')}` };
      const res = await axios.post(`/api/subscription/admin/${userId}/reset-free`, {}, { headers });
      showToast(res.data.message || 'Reset to Free Tier (5 bills/day)');
      fetchData();
    } catch (err: any) {
      showToast(err.response?.data?.error || 'Failed to reset plan', 'error');
    }
  };
