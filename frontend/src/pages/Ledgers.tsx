...
const Ledgers: React.FC = () => {
  ...
  const [statementParty, setStatementParty] = useState<any>(null);
  ...
  const fetchStatement = async (party: any) => {
    setStatementParty(party);
    setIsStatementOpen(true);
    setLoadingStatement(true);
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`/api/ledger/${encodeURIComponent(party.partyName)}/transactions`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setStatementTxns(res.data || []);
    } catch (err) {
      console.error('Error fetching statement', err);
      showToast('Failed to load ledger transactions.', 'error');
    } finally {
                ...
                {filteredLedgers.map((l, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-8 py-5 text-right flex items-center justify-end gap-1.5">
                        <button 
                          onClick={() => fetchStatement(l)}
                          className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all cursor-pointer opacity-100 lg:opacity-0 lg:group-hover:opacity-100"
                ...
                >
                  ...
                  <div className="flex items-center justify-between border-t border-slate-200/60 pt-3">
                    ...
                    <div className="flex gap-2">
                      <button 
                        onClick={() => fetchStatement(l)}
                        className="flex items-center gap-1.5 px-3 py-2 text-[10px] font-black uppercase bg-white border border-slate-200 text-slate-500 hover:text-emerald-600 rounded-xl transition-all shadow-sm"
                      >
        ...
        {isStatementOpen && statementParty && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
            <motion.div 
              ...
              className="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-4xl max-h-[85vh] overflow-hidden flex flex-col"
            >
              {/* Header */}
              <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div>
                  <h3 className="text-base font-black text-slate-900 tracking-tight">
                    Tally Statement: {statementParty.partyName}
                  </h3>
                  <p className="text-[10px] text-slate-400 font-bold mt-0.5 uppercase tracking-wider">
                    {statementParty.gstin ? `GSTIN: ${statementParty.gstin}` : 'NO GSTIN REGISTERED'}
                  </p>
                </div>
                <button 
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {loadingStatement ? (
                  <div className="py-12 flex flex-col items-center justify-center gap-3">
                    ...
                    <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Loading ledger data...</p>
                  </div>
                ) : statementTxns.length === 0 ? (
                  <div className="py-12 text-center space-y-2">
                    <p className="text-sm font-black text-slate-600">No transactions synced yet.</p>
                    <p className="text-xs text-slate-400 font-semibold max-w-md mx-auto">
                      Transactions will appear here once the TallySync agent runs and syncs this company's historical daybook.
                    </p>
                  </div>
                ) : (
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                          ...
                          <th className="px-6 py-4">Voucher Type & No.</th>
                      ...
                      <tbody className="divide-y divide-slate-100 text-xs font-bold text-slate-700 font-mono">
                        {(() => {
                          let bal = 0;
                          return statementTxns.map((t, idx) => {
                            bal += t.amount;
                            const isDr = t.amount > 0;
                            const isCr = t.amount < 0;
                            const drAmt = isDr ? t.amount : 0;
                            const crAmt = isCr ? Math.abs(t.amount) : 0;
                            
                            return (
                              <tr key={t.guid || idx} className="hover:bg-slate-50/50 transition-colors">
                                <td className="px-6 py-3.5 text-slate-500 whitespace-nowrap">{t.date}</td>
                                <td className="px-6 py-3.5">
                                  <span className={`px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${
                                    t.voucherType.toLowerCase() === 'sales' ? 'bg-emerald-50 text-emerald-700' :
                                    t.voucherType.toLowerCase() === 'receipt' ? 'bg-indigo-50 text-indigo-700' :
                                    t.voucherType.toLowerCase() === 'payment' ? 'bg-orange-50 text-orange-700' :
                                    'bg-slate-100 text-slate-700'
                                  }`}>
                                    {t.voucherType}
                                  </span>
                                  <span className="ml-2 font-mono text-slate-900 font-bold">#{t.voucherNumber}</span>
                                </td>
                                <td className="px-6 py-3.5 max-w-xs truncate" title={t.narration}>
                                  <span className="text-slate-800">{t.reference || 'No Ref'}</span>
                                  {t.narration && (
                                    <span className="block text-[10px] text-slate-400 font-sans font-normal truncate mt-0.5">
                                      {t.narration}
                                    </span>
                                  )}
                                </td>
                                <td className="px-6 py-3.5 text-right text-emerald-600">
                                  {isDr ? `₹${drAmt.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '-'}
                                </td>
                                <td className="px-6 py-3.5 text-right text-rose-600">
                                  {isCr ? `₹${crAmt.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '-'}
                                </td>
                                <td className={`px-6 py-3.5 text-right font-black ${bal >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
                                  ₹{Math.abs(bal).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {bal >= 0 ? 'Dr' : 'Cr'}
                                </td>
                              </tr>
                            );
                          });
              ...
              {/* Footer */}
              <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between text-[11px] font-black uppercase text-slate-400 tracking-wider">
                <div>Total Synced Entries: {statementTxns.length}</div>
                <div className="font-mono text-slate-600">
                  Current Balance: ₹{Math.abs(statementParty.balance || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {statementParty.balance >= 0 ? 'Dr' : 'Cr'}
                </div>
              </div>
            </motion.div>
          </div>
