  ...
  const handleJsonParse = () => {
    ...
    try {
      ...
      addToast({
        ...
        message: `Extracted ${extracted.length} invoices from GSTR-2A data.`,
        type: 'success'
      });
    } catch (err: any) {
      addToast({
      ...
      <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 bg-white p-6 md:p-8 rounded-3xl shadow-[0_10px_40px_-20px_rgba(0,0,0,0.02)] border border-slate-200/60 relative overflow-hidden">
          ...
          <div className="space-y-2 relative z-10">
            ...
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">GSTR-2A Reconciliation</h1>
            <p className="text-sm font-semibold text-slate-500">Compare, verify, and sync portal GSTR-2A filings directly into Tally ERP</p>
          </div>
        </div>
            ...
            <div className="bg-white p-4 rounded-2xl shadow-[0_10px_40px_-20px_rgba(0,0,0,0.02)] border border-slate-200/60 flex flex-col md:flex-row md:items-center gap-4">
              ...
              <div className="relative flex-1 md:max-w-xs md:ml-auto">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 placeholder-slate-400 outline-none focus:border-indigo-500 shadow-2xs"
                />
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white/50 p-2 rounded-2xl">
              <div className="flex items-center gap-2 pl-2">
                <span className="text-xs font-bold text-slate-500 bg-white px-3 py-1.5 rounded-full border border-slate-200 shadow-sm">
                  <span className="text-indigo-600 font-black">{filteredEntries.length}</span> Local Purchase Bills • <span className="text-slate-800 font-black">{formatCurrency(filteredEntries.reduce((s, e) => s + (e.totalAmount || 0), 0))}</span> Total
                </span>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-4 bg-white p-6 rounded-3xl border border-slate-200/60 shadow-xs space-y-6">
              <div>
                ...
                <p className="text-xs font-semibold text-slate-500 mt-0.5">Upload Excel sheet or paste GSTR-2A JSON data</p>
              </div>

              {/* Excel upload */}
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Excel GSTR-2A File</label>
                <div className="border-2 border-dashed border-slate-200 rounded-2xl p-4 text-center hover:border-indigo-400 transition-colors relative cursor-pointer group bg-slate-50/50">
                  <input
                    type="file"
                    accept=".xlsx, .xls"
                  ...
                  <div className="flex flex-col items-center justify-center gap-2">
                    <FileSpreadsheet className="h-8 w-8 text-emerald-500 group-hover:scale-110 transition-transform" />
              ...
              {/* JSON Paste area */}
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 tracking-wider">Paste GSTR-2A JSON text</label>
                <textarea
                  placeholder='{"b2b": [{"ctin": "...", "inv": [...]}]}'
                  value={jsonInput}
                  onChange={e => setJsonInput(e.target.value)}
                  className="w-full bg-slate-50 p-4 border border-slate-200 rounded-2xl text-xs font-semibold text-slate-700 min-h-[160px] outline-none font-mono placeholder-slate-350 focus:border-indigo-400 focus:bg-white transition-all resize-none"
                />
                <button
              ...
              {portalBills.length === 0 ? (
                <div className="bg-white rounded-3xl border border-slate-200/60 p-12 text-center text-slate-400">
                  ...
                  <p className="text-xs font-semibold text-slate-500 mt-1 max-w-sm mx-auto">
                    Please paste JSON data or upload an Excel file on the left panel to begin reconciling GSTR-2A filing records with local TallySync bills.
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="bg-white p-5 rounded-2xl border border-slate-200/60 flex flex-col gap-1.5 shadow-2xs">
                      <span className="text-[9px] font-black uppercase text-slate-450 tracking-wider">Total Portal Invoices</span>
                  ...
                  <div className="bg-white p-4 rounded-2xl border border-slate-200/60 flex items-center justify-between shadow-2xs">
                    ...
                    {reconFilter === 'unmatched' && reconSummary.unmatched > 0 && (
                      <button
                        ...
                        className="px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-850 text-white text-xs font-black uppercase tracking-wider rounded-xl shadow-md transition-all active:scale-95 disabled:opacity-50 cursor-pointer flex items-center gap-2"
                      >
