...
import { X, Check, Lock, Zap, Sparkles, Shield, Camera, FileText, ArrowRight, CreditCard, Gift, Loader2 } from 'lucide-react';
...
export default function ProUpgradeModal({
  ...
  const hasClaimedTrial = user?.subscription?.hasClaimedTrial === true || user?.subscription?.plan === 'trial';

  const handleStartFreeTrial = async () => {
    if (hasClaimedTrial) return;

    setClaimingTrial(true);
    try {
      const response = await axios.post(
        '/api/subscription/claim-trial',
        {},
        { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }
      );
      if (response.data.success) {
        await refreshUser();
        showToast('🎉 30-Day Free Trial activated successfully!', 'success');
        onClose();
      }
    } catch (err: any) {
      console.error(err);
      showToast(err.response?.data?.message || err.response?.data?.error || 'Failed to claim free trial', 'error');
    } finally {
          ...
          >
            ...
            <div className="bg-gradient-to-r from-indigo-900 via-indigo-800 to-slate-900 text-white p-8 relative overflow-hidden">
              ...
              <div className="mt-6 flex items-baseline gap-2">
                ...
                <span className="ml-auto text-[10px] font-black uppercase tracking-wider bg-amber-400/20 border border-amber-400/30 text-amber-300 px-3 py-1 rounded-xl">
                  🎁 1-Time 30-Day Free Trial Available
                </span>
              </div>
            </div>
            ...
            <div className="p-6 sm:p-8 space-y-4">
              ...
              <div className="pt-4 space-y-3">
                {/* Razorpay Pay ₹299 Button */}
                <button
                  type="button"
                  onClick={() => setShowRazorpayModal(true)}
                  className="w-full py-4 bg-[#0c2340] hover:bg-[#133054] text-white font-bold rounded-2xl flex items-center justify-center gap-2 text-xs uppercase tracking-wider shadow-lg shadow-blue-900/20 transition-all cursor-pointer"
                >
                  <CreditCard className="w-4 h-4 text-emerald-400" />
                  <span>Pay ₹299/mo with Razorpay & Upgrade</span>
                  <ArrowRight className="w-4 h-4" />
                </button>

                {/* Self-Service 30-Day Free Trial Button */}
                {!hasClaimedTrial ? (
                  <button
                    type="button"
                    onClick={handleStartFreeTrial}
                    disabled={claimingTrial}
                    className="w-full py-4 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-2xl flex items-center justify-center gap-2 text-xs uppercase tracking-wider shadow-lg shadow-emerald-500/20 transition-all cursor-pointer disabled:opacity-50"
                  >
                    {claimingTrial ? (
                      <>
                      ...
                      <>
                        ...
                        <span>Start 30-Day Free Trial (1-Time)</span>
                      </>
                    )}
                  </button>
                ) : (
                  <div className="w-full py-3 bg-amber-50 border border-amber-200 text-amber-800 rounded-2xl flex items-center justify-center gap-2 text-xs font-bold text-center">
                    <Check className="w-4 h-4 text-amber-600" />
                    <span>Free Trial Already Claimed for this Account</span>
                  </div>
                )}
